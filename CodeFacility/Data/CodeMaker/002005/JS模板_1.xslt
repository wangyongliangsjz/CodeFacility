...
function AddData() {
    if (validatorForm() == true) {
        $.ajax({ type: "POST",
            ...
            data: "{<xsl:for-each select="Fields/Field"><xsl:variable name="I" select="position()"/><xsl:choose><xsl:when test="IsIdentity = 'false' and $I = 2"><xsl:value-of select="Name" />:'" + $("#txt<xsl:value-of select="NameDx" />").val() +
                  </xsl:when>
                  <xsl:when test="IsIdentity = 'false'">"',<xsl:value-of select="Name" />:'" + $("#txt<xsl:value-of select="NameDx" />").val() +
                  </xsl:when><xsl:otherwise></xsl:otherwise></xsl:choose></xsl:for-each>"'}",
            dataType: 'json',
    ...
    if (validatorForm() == true) {
        $.ajax({ type: "POST",
            ...
            data: "{<xsl:for-each select="Fields/Field">
                ...
                <xsl:for-each select="Fields/Field">
                  <xsl:if test="PrimaryKey='false'">"',<xsl:value-of select="Name" />:'" + $("#txt<xsl:value-of select="NameDx" />").val() +
                  </xsl:if>
                </xsl:for-each>"'}",
            dataType: 'json',
            success: function (result) {
