...
function QueryData() {
    ...
    <xsl:for-each select="Fields/Field"><xsl:variable name="tjfh"><xsl:choose><xsl:when test="ValueTypeName = 'string' or ValueTypeName = 'char'"><xsl:text> like '%</xsl:text></xsl:when><xsl:otherwise><xsl:text> =</xsl:text></xsl:otherwise></xsl:choose></xsl:variable>if (<xsl:value-of select="Name" /> != "") {
      if (_options.tj != "")
      _options.tj = _options.tj + " and <xsl:value-of select="Name" /><xsl:value-of select="$tjfh" />" + <xsl:value-of select="Name" /><xsl:if test="ValueTypeName = 'string' or ValueTypeName = 'char'"><xsl:text> + "%'"</xsl:text></xsl:if>;
      else
      _options.tj = " <xsl:value-of select="Name" /><xsl:value-of select="$tjfh" />" + <xsl:value-of select="Name" /><xsl:if test="ValueTypeName = 'string' or ValueTypeName = 'char'"><xsl:text> + "%'"</xsl:text></xsl:if>;
    }
    </xsl:for-each>
