...
{
    public partial class FormLabel : FormWin
    {
        ...
        DALFactory.CodeMaker.ILabel dal = new AccessDal.CodeMaker.Label();
        ...
        private void dgv_Data_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            else if (columnindex == 1)  //删除
            {
                ...
                {
                    if (dal.Label_Del(Convert.ToInt32(id)) == 1)
                    {
                        QueryData();
        ...
        {
            IList<LabelInfo> datalist = dal.LabelGetList();
            if (ParentInfo == null)
            {
        ...
        private void GetDataInfo(IList<LabelInfo> datalist, IList<LabelInfo> ilist, LabelTypeInfo ninfo)
        {
            var dlist = from dl in datalist
                        where dl.ParentID == ninfo.ID
                        select dl;
            foreach (LabelInfo dinfo in dlist)
            {
                ilist.Add(dinfo);
            }
        ...
        /// <param name="ninfo">实体</param>
        private void GetMenuInfo(IList<LabelTypeInfo> ilist, LabelTypeInfo ninfo)
        {
            ilist.Add(ninfo);
            var list = from tl in menuList
                       where tl.ParentID == ninfo.ID
