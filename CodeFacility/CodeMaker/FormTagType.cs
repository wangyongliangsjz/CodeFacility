...
{
    public partial class FormTagType : FormWin
    {
        ...
        private void FormTagType_Load(object sender, EventArgs e)
        {
            lb_ID.Text = "";
        ...
        private void dgv_Data_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            else if (columnindex == 1)  //删除
            {
                ...
                {
                    if (menuDal.TagTypeByParentIDGetList(Convert.ToInt32(id)).Count > 0)
                    {
                        MessageBox.Show("请删除子目录。");
                        return;
                    }
                    DALFactory.CodeMaker.ITag tlDal = new AccessDal.CodeMaker.Tag();
                    if (tlDal.TagByParentIDGetList(Convert.ToInt32(id)).Count > 0)
                    {
                        MessageBox.Show("请删除子目录搜索标签。");
                        return;
                    }
                    if (menuDal.TagType_Del(Convert.ToInt32(id)) == 1)
                    {
                        QueryData();
        ...
        private void GetInfo(IList<TagTypeInfo> ilist, TagTypeInfo ninfo)
        {
            ilist.Add(ninfo);
            var list = from tl in menuList
