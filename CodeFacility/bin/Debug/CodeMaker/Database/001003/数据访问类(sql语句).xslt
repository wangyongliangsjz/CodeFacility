...
namespace Maticsoft.Interface_<xsl:value-of select="Name" />
{
  ...
  public class <xsl:value-of select="Name" /> : I<xsl:value-of select="Name" />
  {
    ...
    private const string Sql_SelectModel="Select <xsl:for-each select="Fields/Field">
      <xsl:text disable-output-escaping="yes"></xsl:text>
      <xsl:value-of select="Name" />
      <xsl:if test="position() != last()">,</xsl:if>
    ...
    private const string Sql_SelectAll="Select <xsl:for-each select="Fields/Field">
      <xsl:text disable-output-escaping="yes"></xsl:text>
      <xsl:value-of select="Name" />
      <xsl:if test="position() != last()">,</xsl:if>
    </xsl:for-each> From <xsl:value-of select="Name" />";
    private const string Sql_SelectPagination="";

    private string connString = SqlDataAccess.SQLConnString1;
    ...
    /// 增加一条数据
    ///</summary>
    public int Add(<xsl:value-of select="Name" />Info info)
    {
      int intNum = 0;
    ...
    public bool Delete(int id)
    {
        ...
        <xsl:for-each select="Fields/Field">
        <xsl:if test="IsIdentity='true' or PrimaryKey='true'">
        <xsl:text disable-output-escaping="yes"></xsl:text>param.Add("@<xsl:value-of select="Name" />", id);</xsl:if>
        </xsl:for-each>
    ...
    public DataSet GetModelDs(<xsl:value-of select="$strWhere" />)
    {
      ...
      // 执行语句
      ds = SqlDataAccess.GetDataSet(connString, CommandType.Text, Sql_SelectModel, param);
      return ds;
    }
    ...
    public DataSet GetAllDataSet()
    {
      ...
      DataSet ds = new DataSet();
      // 执行语句
      ds = SqlDataAccess.GetDataSet(connString, CommandType.Text, Sql_SelectAll, null);
      return ds;
    }
    ...
    public IList&lt;<xsl:value-of select="Name" />Info&gt; GetAllList()
    {
      ...
      using (SqlConnection conn = new SqlConnection(connString))
      {
        dr = SqlDataAccess.ExecuteReader(conn, CommandType.Text, Sql_SelectAll, null);
        while (dr.Read())
        {
          <xsl:value-of select="Name" />Info info = new <xsl:value-of select="Name" />Info();
          <xsl:for-each select="Fields/Field">
    ...
    public DataSet GetGrid(string Query, string PageIndex, string PageSize)
    {
      ...
      DataSet ds = new DataSet();
      // 执行语句
      ds = SqlDataAccess.GetDataSet(connString, CommandType.Text, Sql_SelectPagination, param);
      return ds;
    }
    #endregion
