...
namespace Maticsoft.Model_<xsl:value-of select="Name" />
{
  ...
  public class <xsl:value-of select="$classname" />
  {
    ...
    private <xsl:value-of select="ValueTypeName" /><xsl:if test="Nullable = 'true' and ValueTypeName != 'string' and ValueTypeName != 'bool'">?</xsl:if><xsl:text> </xsl:text>_<xsl:value-of select="LcName" /><xsl:if test="DefaultVal != ''">=<xsl:value-of select="DefaultVal" /></xsl:if>;</xsl:for-each>
     <xsl:text>
     </xsl:text>
   <xsl:for-each select="Fields/Field">
