  ...
  public class I<xsl:value-of select="Name" />
  {
    ...
    int Add<xsl:value-of select="Name" />(<xsl:value-of select="Name" />Info info)
    {
      ...
      using (SqlConnection conn = new SqlConnection(SqlDataAccess.SQLConnString1))
      {
        try
        {
          intNum = SqlDataAccess.ExecuteNonQuery(conn, CommandType.StoredProcedure, Sql_<xsl:value-of select="$TableName" />_Insert, param);
        }
        catch
        {
          intNum = 0;
    ...
    int Edit<xsl:value-of select="Name" />(<xsl:value-of select="Name" />Info info)
    {
      ...
      using (SqlConnection conn = new SqlConnection(SqlDataAccess.SQLConnString1))
      {
        try
        {
          intNum = SqlDataAccess.ExecuteNonQuery(conn, CommandType.StoredProcedure, Sql_<xsl:value-of select="$TableName" />_Update, param);
        }
        catch
        {
          intNum = 0;
    ...
    int Del<xsl:value-of select="Name" />(<xsl:value-of select="$strWhere" />)
    {
      ...
      using (SqlConnection conn = new SqlConnection(SqlDataAccess.SQLConnString1))
      {
        try
        {
          intNum = SqlDataAccess.ExecuteNonQuery(conn, CommandType.StoredProcedure, Sql_<xsl:value-of select="$TableName" />_Delete, param);
        }
        catch
        {
          intNum = 0;
