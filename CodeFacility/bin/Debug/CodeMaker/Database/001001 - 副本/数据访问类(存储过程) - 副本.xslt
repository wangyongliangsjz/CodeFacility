...
namespace Maticsoft.Interface_<xsl:value-of select="Name" />
{
  ...
  ///</summary>
  public class I<xsl:value-of select="Name" />
  {
    #region  成员方法
    ...
    DataSet Get<xsl:value-of select="Name" />GetModel(<xsl:value-of select="$strWhere" />)
    {
      ...
      // 执行语句
      ds = SqlDataAccess.GetDataSet(SqlDataAccess.SQLConnString1, CommandType.Text, "Pro_<xsl:value-of select="$TableName" />_GetModel", param);
      return ds;
    }
    <xsl:text>
    </xsl:text>
    <xsl:value-of select="Name" />Info Get<xsl:value-of select="Name" />GetModel(<xsl:value-of select="$strWhere" />)
    {
      ...
      using (SqlConnection conn = new SqlConnection(SqlDataAccess.SQLConnString1))
      {
        dr = SqlDataAccess.ExecuteReader(conn, CommandType.Text, "Pro_<xsl:value-of select="$TableName" />_GetModel", param);
        if (dr.Read())
        {
          <xsl:for-each select="Fields/Field">
            <!-- 定义默认值 -->
    ...
    DataSet Get<xsl:value-of select="Name" />List()
    {
      ...
      DataSet ds = new DataSet();
      // 执行语句
      ds = SqlDataAccess.GetDataSet(SqlDataAccess.SQLConnString1, CommandType.Text, "Pro_<xsl:value-of select="$TableName" />_GetList", null);
      return ds;
    }
    ...
    IList&lt;<xsl:value-of select="Name" />Info&gt; Get<xsl:value-of select="Name" />List()
    {
      ...
      using (SqlConnection conn = new SqlConnection(SqlDataAccess.SQLConnString1))
      {
      dr = SqlDataAccess.ExecuteReader(conn, CommandType.Text, "Pro_<xsl:value-of select="$TableName" />_GetList", null);
      while (dr.Read())
      {
        <xsl:value-of select="Name" />Info info = new <xsl:value-of select="Name" />Info();
        <xsl:for-each select="Fields/Field">
    ...
    DataSet GetList(int PageSize,int PageIndex,string strWhere)
    {
      ...
      DataSet ds = new DataSet();
      // 执行语句
      ds = SqlDataAccess.GetDataSet(SqlDataAccess.SQLConnString1, CommandType.Text, "Pro_<xsl:value-of select="$TableName" />_GetListPagination", null);
    return ds;
    }
    #endregion  成员方法
    }
